...
!function(A) {
    ...
    mboxFactoryDefault.addOnLoad(function(){
        // Adbett.track({selector:'.appdl-badges a:eq(0)',value:'apple-button',event:'mousedown',type:'signaler'});
        // Adbett.track({selector:'.appdl-badges a:eq(1)',value:'google-button',event:'mousedown',type:'signaler'});

        jQuery(".checkRates .bookAnchor").on('mouseover', function () {
            if ( jQuery(this) ) {
                mboxFactoryDefault.getSignaler().signal("mboxClickTrack", "mboxClickTrack", "clicked=check-rates-hover");
            }
        });
    });
