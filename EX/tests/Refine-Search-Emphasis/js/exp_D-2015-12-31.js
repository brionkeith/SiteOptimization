jQuery(document).ready(function (jQuery) {
	...
	// BUILD SHOW/HIDE TRIGGER
	jQuery('label.spanOffset').on('click', function(event) {
		jQuery(this).addClass('taoShow');
		event.preventDefault();
		if ('.col-md-12') {
		    jQuery('.taoRatings, .taoAmenities, .taoBrands, .taoFilterBy, #btnApplyFilter').toggle(taoShow taoHide);
		}
	});

	// jQuery('label.spanOffset').on('click', (function(event) {
	// 	jQuery('label.spanOffset').toggleClass('taoShow taoHide');
	// });

	// jQuery('.col-md-12 label.spanOffset').on('click', function(event) {
	// 	jQuery(this).removeClass('taoShow');
	// });
	...
	jQuery('#btnApplyFilter').insertAfter('.taoFilterBy');
