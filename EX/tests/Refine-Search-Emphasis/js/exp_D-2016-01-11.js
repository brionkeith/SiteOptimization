jQuery(document).ready(function(jQuery) {
  ...
  jQuery(document).ajaxComplete(function(event, request, settings) { // Request AJAX
    ...
    // REMOVE DUPLICATE DIVs and BUTTON INPUT
    // jQuery('#btnApplyFilter:eq(0) + .taoAmenities, #btnApplyFilter:eq(0) + .taoBrands, #btnApplyFilter:eq(0) + .taoFilterBy').remove();
    // jQuery('#btnApplyFilter:eq(0) ~ #btnApplyFilter:eq(1), #btnApplyFilter:eq(0) ~ #btnApplyFilter:eq(2), #btnApplyFilter:eq(0) ~ #btnApplyFilter:eq(3), #btnApplyFilter:eq(0) ~ #btnApplyFilter:eq(4)').remove();
  ...
  jQuery(document).ajaxStop((function() {
    ...
    jQuery('.taoFilterParent').text("AJAX request stopped.");
  });
