// Bootstrapper.MVT.injectCSS('.priceInfoArea .row:nth-child(1), .priceInfoArea .row:nth-child(2), .priceInfoArea .row:nth-child(3), ' +
//     '.priceInfoArea .row:nth-child(4), .priceInfoArea .row:nth-child(5){ display:none; }');

// Hide default content
Bootstrapper.MVT.injectCSS('' +
    '.priceInfoArea .priceBefore, ' +
    '.priceInfoArea .priceBookSave, ' +
    '.priceInfoArea .priceNow, ' +
    '.priceInfoArea .avgrate,' +
    '.priceInfoArea .vatText ' +
    '{ display:none; }');
...
function taoTripTeaseModule() {
    ...
    jQuery('.row.trip-tease-container').each(function () {
        ...
        jQuery(this).replaceWith(
            ...
            'data-pf-custom-css="https://prodcache.internal.ihg.com/content/dam/etc/media_library/cn/0/cn/css/sr/tao-triptease-exp-D.css">' +
            '</div>');
    ...
    jQuery('.priceInfoArea').each(function () {
        ...
        // Change input .val() to "Check Our Rates"
        jQuery('.checkRates .btn-primary').val('Check Our Rates');

        // Move 'price-fighter-widget' into each '.priceInfoArea' container
        jQuery(this).find('.price-fighter-widget').appendTo(jQuery(this));

        // For each '.checkRates' button
        // insert it after the '.price-fighter-widget'
        jQuery('.checkRates').each(function () {
            var item = jQuery(this);
            item.insertAfter(item.next());
        });

        //////////////////////////// CONTROL WHEN AND WHEN NOT TO DISPLAY PRICE INFO ////////////////////////////

        // If these selectors have both child selectors, display them
        if(jQuery('.priceInfoArea .row:eq(0), .priceInfoArea .row:eq(1)')){
            jQuery(this).has('div.priceMsgNotAvail' && 'div.priceMsg').css('display', 'block');
        }

        // If 'priceInfoArea' does not contain 'price-fighter-widget' display price info
        var taoPriceInfo  = jQuery('.priceInfoArea:not(:has(.price-fighter-widget)) ');
        if(jQuery('.priceInfoArea').not('.trip-tease-container')) {
            jQuery(taoPriceInfo).addClass('no-widget');
        }
