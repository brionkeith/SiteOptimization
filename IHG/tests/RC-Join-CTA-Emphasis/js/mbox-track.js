...
!function(A) {
    ...
    mboxFactoryDefault.addOnLoad(function(){
        Adbett.track({selector:'.contentWrapperHeader .marketingModuleState #joinNowSubmit',value:'joinnow',event:'click',type:'signaler'});
        Adbett.track({selector:'.contentSlots .marketingModuleState #joinNowSubmit',value:'joinnow',event:'click',type:'signaler'});

        jQuery('.signInBtn.row #pcrLeftSignInBtn').on('click', function () {
            if (jQuery('.UHF_pcrDialogRow .pcrReqdDefault.pcrReqdField , .UHF_pcrDialogRow .pcrReqdDefault')) {
                // Track when 'pcrDialogRow' sign-in fields are populated and reservation confirmation button is clicked
                mboxFactoryDefault.getSignaler().signal("mboxClickTrack", "mboxClickTrack", "clicked=signin");
            }
        });
    });
