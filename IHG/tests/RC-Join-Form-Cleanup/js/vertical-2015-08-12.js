jQuery(document).ready(function () {
    ...
    // jQuery('.taoAccountBlock:eq(4)').empty(); // Remove 'confirm email' field
    ...
    // DECISION WAS MADE TO NOT WORRY ABOUT THE FORM FIELDS (I.E. STATE/COUNTRY/PROVINCE)
    jQuery('.taoMailingBlock select#countrySelect').on('change', function () {
        if (jQuery('.taoMailingBlock select#stateSelect')) {
            alert("Handler for .change() called.");
            jQuery('<label>State / County / Province: <span>*</span></label>').insertBefore('.taoMailingBlock #stateSelect'); // Add missing label
            jQuery('.taoMailingBlock #stateSelect option:eq(0)').text("Select One");
        }
    });
