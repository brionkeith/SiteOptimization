jQuery(document).ready(function () {
    ...
    var taoRcHeroTxt = jQuery('<section><h3>Some Benefits of Membership include:</h3></section>'); // Hero heading
    ...
    jQuery('#rcBenefits li').each(function (index) { // add each array item to list
    ...
    jQuery('.checkbox.section').prepend('<h3>Terms and Conditions</h3>'); // Prepend header text to terms component
    jQuery('.inline p').html('I have reviewed and accept the <a href="https://www.ihg.com/hotels/us/en/global/customer_care/member-tc" target="_new">Terms and Conditions</a>. I also certify that am at least 18 years old and of lawful age.'); // Overwrite terms and conditions text
    ...
    */
    ...
    jQuery('<label>Title: <span>*</span></label>').prependTo('#taoAccountInfo .taoAccountBlock:eq(0)'); // Change label text, include span with "*"
    ...
    jQuery("#taoAccountInfo .taoAccountBlock").last().addClass("taoClear");
    jQuery('<label>Create PIN: (PIN should be 4 digits)<span>*</span></label>').prependTo('#taoAccountInfo .taoAccountBlock:eq(4)');
    ...
    jQuery('<label>Verify PIN: <span>*</span></label>').prependTo('#taoAccountInfo .taoAccountBlock:eq(5)');
    ...
    // Remove 'account info' and 'mailing address' headings
    jQuery('#formHeaderWrapper, #taoMailingAddr h2').remove();
    ...
    jQuery("#taoMailingAddr .taoMailingBlock input").last().attr("placeholder", taoExistingStreetLabel2);
    jQuery("#taoMailingAddr .taoMailingBlock").last().addClass("taoFlushRight");
    ...
    jQuery("#taoMailingAddr .taoMailingBlock").last().addClass("taoFlushRight");
    jQuery('<label>City / Town: <span>*</span></label>').prependTo('#taoMailingAddr .taoMailingBlock:eq(3)');
    ...
    jQuery("#taoMailingAddr .taoMailingBlock").last().addClass("taoClear");
    jQuery('<label>State / County / Province: <span>*</span></label>').prependTo('#taoMailingAddr .taoMailingBlock:eq(4)');
    ...
    jQuery('.taoMailingBlock.taoClear').remove(); // Remove first table row containing "address type" radio buttons
